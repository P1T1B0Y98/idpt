...
export const FormItemRenderer = ({ formItem, decorator, initialValue }) => {
  ...
  if (type === 'select') {
    return (
      <Form.Item label={label} help={help}>
        {decorator(field, { ...fieldProps, initialValue: initialValue || [] })(
          <Select {...fieldProps}>
            {fieldProps.options && fieldProps.options.map(item => (
  ...
  return (
    ...
    >
      {
        decorator(field, { ...fieldProps, initialValue })(
...
const FormRenderer = (
  {
    ...
    onSave,
    onError,
    formStructure: { id, type, title, formSchema },
    formProps,
    allowDraft = true,
    allowSubmit = true,
    disabled = false
      ...
      <Col>
        <Row>
          ...
          <fieldset disabled={disabled}>
            {!isEmpty(formSchema) && formSchema.map((fieldItem, index) => (
              <Row key={index}>
                <FormItemRenderer
                  formItem={fieldItem}
...
export default Form.create({ name: 'form_renderer' })(FormRenderer)
