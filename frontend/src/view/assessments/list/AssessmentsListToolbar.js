...
class AssessmentToolbar extends Component {
  doExport = () => {
    ...
    return (
      <Toolbar>
        ...
        {this.props.hasPermissionToAuditLogs && (
          <Link to="/audit-logs?entityNames=assessments">
...
function select(state) {
  return {
    ...
    hasPermissionToCreate: assessmentsSelectors.selectPermissionToCreate(
      state,
    ),
    hasPermissionToImport: assessmentsSelectors.selectPermissionToImport(
      state,
    ),
  };
}

export default connect(select)(AssessmentToolbar);
