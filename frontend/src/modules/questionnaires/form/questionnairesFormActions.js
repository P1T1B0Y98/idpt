...
const actions = {
  ...
  doFind: id => async dispatch => {
    try {
      ...
      const record = await QuestionnairesService.find(id)
  ...
  doCreate: values => async dispatch => {
    try {
      ...
      await QuestionnairesService.create(values)
  ...
  doUpdate: (id, values) => async (dispatch, getState) => {
    try {
      ...
      await QuestionnairesService.update(id, values)

      dispatch({ type: actions.UPDATE_SUCCESS })

      Message.success(i18n('entities.questionnaires.update.success'))

      getHistory().push(`/questionnaires/${id}/edit`)
    } catch (error) {
      Errors.handle(error)
