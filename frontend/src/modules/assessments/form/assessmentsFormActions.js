...
const actions = {
  ...
  doFind: id => async dispatch => {
    try {
      ...
      const record = await AssessmentsService.find(id)
  ...
  doCreate: values => async dispatch => {
    try {
      ...
      await AssessmentsService.create(values)

      dispatch({ type: actions.CREATE_SUCCESS })

      Message.success(i18n('entities.assessments.create.success'))
  ...
  doUpdate: (id, values) => async (dispatch, getState) => {
    try {
      ...
      await AssessmentsService.update(id, values)

      dispatch({ type: actions.UPDATE_SUCCESS })

      Message.success(i18n('entities.assessments.update.success'))

      getHistory().push(`/assessments/${id}/edit`)
    } catch (error) {
      Errors.handle(error)
