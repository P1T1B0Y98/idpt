...
/**
 * Handles database operations for the Assessment.
...
class AssessmentRepository {
  ...
  /**
   * Counts the number of Assessments based on the filter.
   *
   * @param {Object} filter
   * @param {Object} [options]
   */
  ...
  /**
   * Finds the Assessment and its relations.
   *
   * @param {string} id
   * @param {Object} [options]
   */
  async findById (id, options) {
    return MongooseRepository.wrapWithSessionIfExists(
      Assessment.findById(id).populate('owner'),
      options
    )
  }

  /**
   * Finds the Assessments based on the query.
  ...
  async findAndCountAll (
    ...
    const count = await Assessment.countDocuments(criteria)

    return { rows, count }
  }

  /**
   * Lists the Assessments to populate the autocomplete.
  ...
  async _createAuditLog (action, id, data, options) {
    await AuditLogRepository.log(
      { entityName: Assessment.modelName, entityId: id, action, values: data },
      options
    )
  }
}
